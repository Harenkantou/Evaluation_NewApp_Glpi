...
function validateColumns(csvText, expectedKey) {
  ...
  // Aucune ligne du tout (pas même un en-tête) -> vrai fichier vide
  if (!lines.length) return 'Fichier totalement vide (aucun en-tête)'
  ...
  // NB : un fichier avec en-têtes mais SANS données est VALIDE (retour null).
  //      Il sera simplement importé comme 0 ligne (voir hasDataRows / parseCSV).
  return null
}

// Indique si un CSV contient au moins une ligne de DONNÉES (au-delà de l'en-tête).
// false = fichier "en-têtes seulement" (vide de données) -> cas géré, non bloquant.
...
export async function importAll({ feuille1, feuille2, feuille3, images = null, onProgress }) {
  ...
  try {
    ...
    // Validation : un fichier doit AU MINIMUM avoir les bons en-têtes.
    // (un fichier "en-têtes seulement" passe la validation -> accepté)
    ...
    // Détection des fichiers "en-têtes seulement" (0 ligne de données).
    // Ce n'est PAS une erreur : on saute simplement l'import correspondant.
    const skipped = []
    const hasElements = hasDataRows(text1)
    const hasTickets = hasDataRows(text2)
    const hasCosts = hasDataRows(text3)
    if (!hasElements) skipped.push('Éléments (fichier vide : en-têtes seulement)')
    if (!hasTickets) skipped.push('Tickets (fichier vide : en-têtes seulement)')
    if (!hasCosts) skipped.push('Coûts (fichier vide : en-têtes seulement)')

    const refCache = makeRefCache()

    // 1) Éléments — uniquement s'il y a des données
    let createdElements = []
    if (hasElements) {
      onProgress?.({ phase: 'elements', message: 'Import des éléments...', done: 0, total: 100 })
      createdElements = await importElements(text1, sessionToken, refCache, (p) => {
        if (p.batch) trackCreated(p.batch)
        onProgress?.(p)
      })
      trackCreated(createdElements)
    } else {
      onProgress?.({ phase: 'elements', message: 'Éléments : aucun (en-têtes seulement)' })
    }

    // 2) Tickets — uniquement s'il y a des données
    //    (les tickets référencent des éléments ; sans éléments, les liaisons
    //     seront simplement ignorées, ce qui est cohérent)
    let createdTickets = []
    if (hasTickets) {
      onProgress?.({ phase: 'tickets', message: 'Import des tickets...', done: 0, total: 100 })
      createdTickets = await importTickets(text2, sessionToken, createdElements, onProgress)
      for (const t of createdTickets) if (t?.id) allCreated.push({ itemtype: 'Ticket', id: t.id })
    } else {
      onProgress?.({ phase: 'tickets', message: 'Tickets : aucun (en-têtes seulement)' })
    }

    // 3) Coûts — uniquement s'il y a des données
    let createdCosts = []
    if (hasCosts) {
      onProgress?.({ phase: 'costs', message: 'Import des coûts...', done: 0, total: 100 })
      createdCosts = await importCosts(text3, sessionToken, createdTickets, onProgress)
      for (const c of createdCosts) if (c?.id) allCreated.push({ itemtype: 'TicketCost', id: c.id })
    } else {
      onProgress?.({ phase: 'costs', message: 'Coûts : aucun (en-têtes seulement)' })
    }
    ...
    return {
      ...
      message: skipped.length
        ? 'Import terminé (certains fichiers étaient vides)'
      ...
      details: {
        ...
        images: imageResult
          ...
          : 'Non importées',
        // Liste des fichiers ignorés car vides (en-têtes seulement)
        skipped
      }
    }
  } catch (error) {
