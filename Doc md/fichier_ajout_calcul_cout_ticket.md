...
> ## ✅ STATUT : IMPLÉMENTÉ (build Vite OK — 105 modules, 0 erreur)
> Les 6 fichiers ci-dessous ont été créés/modifiés dans
> `newapp/frontend/src/` **et** copiés dans `uploads/`.
> Formule vérifiée sur les données réelles (ticket #1) → **160,45 €**.
>
> | # | Fichier | État |
> |---|---|---|
> | 1 | `services/glpiApi.js` | ✅ `getTicketCosts()` ajouté |
> | 2 | `services/cost.js` | ✅ créé (formule) |
> | 3 | `views/frontoffice/KanbanView.vue` | ✅ coût dans la modale détails |
> | 4 | `stores/glpi.js` | ✅ `totalCost` dans `fetchStats()` |
> | 5 | `views/backoffice/DashboardView.vue` | ✅ carte « Coût total tickets 💰 » |
> | 6 | `views/backoffice/TicketsView.vue` | ✅ colonne « Coût total » |
>
> ⚠️ **À valider chez toi** : que l'endpoint `/Ticket/{id}/TicketCost`
> renvoie bien les lignes (sinon utiliser la variante section 4.1).
> Vérification SQL : section 5.

---

> **Constat initial** : la Feuille 3 (coûts) était bien **importée** dans GLPI
> (table `glpi_ticketcosts` via `importCosts` dans `import.js`), mais
> les coûts n'étaient **jamais relus ni affichés** dans NewApp.
> Ce guide liste **tous les fichiers corrigés** + la **formule** + le code.
...
> C'est la version **implémentée** (testée en compilation).
>
> **Variante** si la sous-ressource renvoie vide chez toi :
> ```js
...
Détails importants
Formule appliquée : cost_fixed + cost_material + (actiontime/3600) × cost_time, sommée sur toutes les lignes du ticket.
Vérifiée sur tes données réelles (ticket #1, 2 lignes) → 160,45 € ✅
Les fichiers de base (glpiApi.js, glpi.js, DashboardView.vue, KanbanView.vue) ont été synchronisés depuis uploads/ (versions les plus récentes) avant modification, donc tout est cohérent dans newapp/frontend/src/.
Les appels de coûts utilisent .catch(() => []) pour qu'un ticket sans coût n'affiche jamais d'erreur (juste 0.00 €).

// dans glpiApi.js, variante si la sous-ressource ne marche pas
const { data } = await legacy.get('/TicketCost', {
  headers: { 'Session-Token': session },
  params: { 'searchText[tickets_id]': ticketId, expand_dropdowns: false }
})